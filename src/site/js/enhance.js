(() => {
  const root = document.querySelector('.enhance-page');
  if (!root) return;
  ...
  const apiLinkifyUrl = `${apiBase}/api/linkify`;
  ...
  if (!form || !codeInput || !output || !functionList || !statusEl || !submitBtn || !clearBtn || !copyBtn) {
    return;
  }
  ...
  const handleSubmit = async event => {
    ...
    try {
      const payload = { code, language: requestLanguage };

      const response = await fetch(apiLinkifyUrl, {
        method: 'POST',
